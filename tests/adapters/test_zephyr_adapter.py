...
class TestCaching:
    def test_cache_keyed_by_parent(self, adapter, fake_redis):
        with patch("requests.Session.get", zephyr_requests_get({"BLOCK-T123": "Parent case"})):
    ...
    def test_only_missing_parents_are_fetched(self, adapter):
        with patch("requests.Session.get", zephyr_requests_get({"BLOCK-T1": "First case"})):
            adapter.fetch_by_ids(["BLOCK-T1"])
    ...
    def test_none_result_is_cached(self, adapter):
        with patch("requests.Session.get", zephyr_requests_get({})):
