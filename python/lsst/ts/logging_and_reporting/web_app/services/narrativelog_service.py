...
def get_messages(
    dayobs_start: int, dayobs_end: int, telescope: str, auth_token: str = None
) -> list:
    """
    ...
    if status.get("error") is not None:
        raise Exception(
            f"Error getting narrative log records from {status.endpoint_url}: {status.error}"
        )
    records = narrative_log.records
    instrument_records = [
        record for record in records if record.get("instrument") == telescope
    ]
