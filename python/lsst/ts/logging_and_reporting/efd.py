...
class EfdAdapter(SourceAdapter):
    def __init__(self, *,
        ...
        self.client = None
        self.server_url = server_url or os.getenv('EXTERNAL_INSTANCE_URL',
                                                  Server.usdf)
        match self.server_url:
            ...
            case Server.tucson:
                pass
            case _:
                msg = (f'Unknown server from EXTERNAL_INSTANCE (env var).  '
    ...
    async def find_populated_topics(self, days=1, max_topics=None):
        ...
        print(f'Query all {len(all_topics)} topics '
        ...
        for topic in all_topics:
            ...
            try:
                fields = [f for f in await self.client.get_fields(topic)
