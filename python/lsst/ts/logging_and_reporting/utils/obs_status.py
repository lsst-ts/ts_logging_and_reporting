...
"""Stateless helpers for observatory-status intervals and metrics.

The observatory status is a bitmask of `OBSERVATORY_STATES`. These helpers
decode it, turn a sequence of status events into intervals, and sum the
overlap of those intervals with night or dayobs windows to produce metrics.
The `ObsStatusService` orchestrates them.
"""
...
def dayobs_to_unix_ms(dayobs: int, hour: int = 12) -> int:
    ...
    date = datetime.strptime(str(dayobs), "%Y%m%d").replace(hour=hour, tzinfo=timezone.utc)
